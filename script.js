...
themeBtn.addEventListener("click", () => {
  ...
  if (currentTheme === "dark") {
    ...
    document.body.style.backgroundColor = "hsl(200, 60%, 99%)";
...
function displayExtensions(extensions) {
  ...
  extensions.forEach((extension) => {
    ...
    item.innerHTML = `
        <div class="top">
            <img src=${extension.logo} alr="${extension.name}">
      ...
      <div class="toggle-switch">
        <input  class="toggle-input" 
            ...
            <label for="toggle-${extension.name}" class="toggle-label"></label>
      </div>
    </div>
    `
...
function handleToggle(e) {
  if(e.target.classList.contains('toggle-input')){
    ...
    if(extension) {
      extension.isActive = e.target.chekced;
    }
  }
}
